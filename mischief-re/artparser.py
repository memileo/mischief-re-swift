...
result = unwrap_pressure_sequence(raw_ps)

# Expected: [498.0, 1213.0, 1797.0, 2218.0]
...
def read_action(data, pos):
    ...
    if val['action_id'] == 0x01:
        ...
        # --- read first absolute point ---
        (x0, pos) = read_float(data, pos)
        (y0, pos) = read_float(data, pos)
        (p0_float, pos) = read_float(data, pos)
        
        xs = [x0]
        ys = [y0]
        
        # FIX: Multiply by 4096 for the 12-bit range anchor
        first_p_raw = int(p0_float * 4096)
        raw_ps = [first_p_raw]
        ...
        for i in range(point_count - 1):
            ...
            # 10-bit raw packed pressure
            p_raw = ((tmp >> 30) & 0x3) | ((byt & 0xFF) << 2)
            
            x0 += dx/32.0
            y0 += dy/32.0
            xs.append(x0)
            ys.append(y0)
            raw_ps.append(int(p_raw))

        # Remove last point 0 with duplicated coordinates
        del raw_ps[-1]
        print("raw_ps: ", raw_ps)
        # Perform the unwrap
        unwrapped = unwrap_pressure_sequence(raw_ps)
        print("unwrapped: ", unwrapped, "\n")

        points = []
        for xi, yi, pi, rp in zip(xs, ys, unwrapped, raw_ps):
            points.append({
                'x': xi, 
                'y': yi, 
                'p': pi / 4095.0, # Normalizing to 1.0
                'raw_p': rp,
                'unwrapped': pi
            })
