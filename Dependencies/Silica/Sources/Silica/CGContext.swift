...
public final class CGContext {
    ...
    internal let internalContext: Cairo.Context
...
//     }
    ...
    public func restoreGState() {
        ...
        // Check if we have a pending mask
        if let maskInfo = pendingMasks.popLast() {

            // Pop the offscreen group
            // We assume the stack is balanced because we only added to pendingMasks
            // if pushGroup was called successfully.
            internalContext.popGroupToSource()

            // CHECK: Use the current Cairo operator directly
            // This matches the mode used by the drawing operations just performed.
            if internalContext.operator == CAIRO_OPERATOR_DEST_OUT {

                // --- ERASER LOGIC ---

                let targetWidth = Int(maskInfo.rect.width)
                let targetHeight = Int(maskInfo.rect.height)
                ...
                // 1. Create scaled mask
                let scaledMaskSurface = try! Surface.Image(format: .argb32, width: targetWidth, height: targetHeight)
                let tempContext = Context(surface: scaledMaskSurface)

                let scaleX = Double(targetWidth) / Double(maskInfo.width)
                let scaleY = Double(targetHeight) / Double(maskInfo.height)

                tempContext.scale(x: scaleX, y: scaleY)
                let sourcePattern = Pattern(surface: maskInfo.surface)
                tempContext.source = sourcePattern
                tempContext.paint()

                // 2. Erase using the mask
                internalContext.save()

                // Set opaque color source
                internalContext.source = Cairo.Pattern(color: (red: 1, green: 1, blue: 1))

                // Set operator to remove content
                internalContext.operator = CAIRO_OPERATOR_DEST_OUT

                // Apply mask
                internalContext.mask(surface: scaledMaskSurface, at: (x: maskInfo.rect.origin.x, y: maskInfo.rect.origin.y))
                ...
                // Paint the recorded group masked by the surface
                internalContext.mask(surface: scaledMaskSurface, at: (x: maskInfo.rect.origin.x, y: maskInfo.rect.origin.y))
            }
        }

        // Restore the main Cairo state
        try! restore()
    }
    ...
    // MARK: - Pending mask state (add as stored properties on CGContext)

    /// Tracks in-flight image-mask clips. Each entry corresponds to a
    /// clip(to:mask:) call that pushed a group onto the Cairo group stack.
    /// Consumed by the matching restoreGState().
    private var pendingMasks: [(surface: Surface.Image, width: Int, height: Int, rect: CGRect)] = []


    // MARK: - clip(to:mask:)

    public func clip(to rect: CGRect, mask: CGImage) {
        // 1. Apply the rectangular clip (path-based — Cairo handles this natively)
        self.clip(to: rect)

        // 2. Push a group — all subsequent drawing is recorded into an
        //    offscreen surface instead of going directly to the target.
        internalContext.pushGroup()

        // 3. Stash the mask info so restoreGState() can apply it.
        //    Swift ARC keeps the Surface.Image alive; no manual reference needed.
        pendingMasks.append((
...
//         internalContext.operator = CAIRO_OPERATOR_OVER
        ...
        // Check if we have transparency to apply
        let currentAlpha = Double(internalState.alpha)

        if currentAlpha >= 1.0 {
            // Opaque: Standard fill
            internalContext.fill()
        } else {
            // Transparent:
            // 1. Clip to the rectangle path (so we don't paint the whole surface)
            internalContext.clip()
            // 2. Paint the pattern with the specific alpha
            internalContext.paint(alpha: currentAlpha)
        }
    ...
    public func fill(_ rect: CGRect) {
        // 1. Reset path to avoid drawing previous shapes
        internalContext.newPath()

        // 2. Define the shape
        addRect(rect)

        // 3. FIX: Manually apply the color from internalState to the raw context.
        // The 'fillColor' setter saves it to internalState, but doesn't push it to Cairo
        // until a draw call happens. Since we are calling internalContext.fill() directly,
        // we must set the source manually.
        if let fillState = internalState.fill {
            // fillState is a tuple: (CGColor, Cairo.Pattern)
            // We need the Pattern (index 1) to set the source.
            internalContext.source = fillState.1
        } else {
            // Optional: Handle nil case if necessary, though usually setFillColor is called first.
            // If internalState.fill is nil, it might default to black anyway.
        }

        // 4. Fill using the raw context (now that it has the correct color)
        internalContext.fill()
    }
